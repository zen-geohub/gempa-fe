...
function Map3D() {
  ...
  useEffect(() => {
    ...
    setEarthquakeData({
      latitudes: filteredEarthquake.map(
        (feature) => feature.geometry.coordinates[1]
      ),
      longitudes: filteredEarthquake.map(
        (feature) => feature.geometry.coordinates[0]
      ),
      depth: filteredEarthquake.map(
        (feature) => feature.properties.depth_km * -1
      ),
      magnitude: filteredEarthquake.map(
    ...
    <Plot
      data={[
        ...
        },
        {
          ...
          customdata: earthquakeData.magnitude,
          hovertemplate:
            "Bujur: %{x}<br />" +
            "Lintang: %{y}<br />" +
            "Kedalaman (km): %{z}<br />" +
            "Magnitudo: %{customdata}",
          marker: {
            size: filteredEarthquake.map((feature) => {
              if (feature.properties.magnitude_class === "Gempa Kecil") {
                return 5;
              } else if (
                feature.properties.magnitude_class === "Gempa Menengah"
              ) {
                return 8;
